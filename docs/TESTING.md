...
2. Application: hash the official APK, build `.mpp`, select one patch in exclusive mode, and require all fingerprints to resolve once.
3. Inspection: verify activity and metadata in the manifest, injected classes, preference hook, and cue hook.
4. Phone Manager: add the private source with a user-supplied PAT on `Pixel_10`, select one patch, export using the dedicated test key policy, and install the result on the TV target.
