...
The patch adds `Settings → Morphe → Subtitles` with three native Nuvio rows: `Off`, `Remove SDH, keep lyrics`, and `Full cleanup`. It defaults to `Off` and stores the selected mode in private `morphe_patches` preferences under `subtitles.sdh_cleanup_mode`.
